...
python -c "import fastapi, uvicorn, multipart" >/dev/null 2>&1 || {
    echo "❌ Missing Real-ESRGAN server dependencies in $VENV"
...
if pgrep -f "realesrgan_server.py" > /dev/null; then
    if curl -s --max-time 2 "http://127.0.0.1:$PORT/" > /dev/null 2>&1; then
        ...
        echo "⚠️  Found stale Real-ESRGAN process without a listening port; stopping it..."
        pkill -f "realesrgan_server.py" || true
        sleep 2
